...
void FilterBitSet::addInput(RowVectorPtr input) {
...
bool FilterBitSet::allInputProcessed() {
  if (!input_) {
    return true;
  }
...
bool FilterBitSet::isFinished() {
  return noMoreInput_ && allInputProcessed();
}

RowVectorPtr FilterBitSet::getOutput() {
...
void FilterBitSet::project(const SelectivityVector& rows, EvalCtx& evalCtx) {
  exprs_->eval(
      hasFilter_ ? 1 : 0, numExprs_, !hasFilter_, rows, evalCtx, results_);
}

vector_size_t FilterBitSet::filter(
    EvalCtx& evalCtx,
    const SelectivityVector& allRows) {
  auto start = std::chrono::steady_clock::now();
  exprs_->eval(0, 1, true, allRows, evalCtx, results_);
  std::cout << "FilterBitSet::filter cost"
            << std::chrono::duration_cast<std::chrono::milliseconds>(
                   std::chrono::steady_clock::now() - start)
                   .count()
            << "ms" << std::endl;
